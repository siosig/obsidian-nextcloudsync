...
- **Reach any action in one tap by pinning it to the mobile toolbar.** Obsidian hides the ribbon entirely on mobile, so plugin ribbon icons never appear there — its own icons don't either. Commands are the route instead: open **Settings → Toolbar** and add **Sync now**, **Open sync status** or **Mirror from remote**. Pinned, each is a single tap; from the command palette, two. **Open sync status** is worth pinning first: the dialog it opens holds both **Sync now** and **Mirror from remote**.
