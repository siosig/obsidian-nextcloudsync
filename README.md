...
## What's new in this release (1.0.4)

- **Fixed: deleting the last note in a Vault never reached your other devices (1.0.4)** — reported in [#50](https://github.com/siosig/obsidian-nextcloudsync/issues/50). When a full sync found the server listing empty, the plugin skipped its "these files are gone from the server, remove them here too" step entirely. If every file it was tracking had been deleted elsewhere — easiest to hit in a small Vault — the notes stayed on disk, and because they also stayed in the plugin's index, the next sync rebuilt the listing from that index and concluded they were still on the server. It never recovered on its own. The two protections that matter are unchanged: a limit that refuses a suspiciously large batch of deletions, and a direct check with the server for every single candidate before anything is removed.
- **Fixed: a Vault folder deleted on the server is now put back, not obeyed (1.0.4)** — the same investigation found that if the Vault folder itself was missing from the server, the plugin did nothing at all: files were neither uploaded nor removed, and the Vault sat in limbo. A missing folder says nothing about any individual file, and you never chose that folder's remote path — the plugin derives it from the Vault name. So it is now repaired: the folder is re-created and this Vault re-uploaded from your device, deleting nothing locally. It only does so once the server confirms the folder really was absent; if it turns out to be there after all, nothing is changed and the next sync tries again.
- **Fixed: an empty folder was not put back when the Vault folder had to be re-created (1.0.4)** — the plugin keeps a note of which folders it has already created on the server, and that note outlived the folder it described, so the request to restore an empty one was skipped as redundant. Files were unaffected because uploading into a missing folder fails, and that failure already forced a retry; an empty folder has no upload to fail.
- **Development toolchain refreshed (1.0.4)** — everything that tests, lints and bundles the plugin moved to its current release, and the packaged output came out byte-for-byte identical.
