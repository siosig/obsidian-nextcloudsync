...
## What's new in this release (1.0.5-beta.2)

- **Fixed: a listing the server could not answer properly was read as "there is nothing here" (1.0.5-beta.2)** — a follow-up to [#51](https://github.com/siosig/obsidian-nextcloudsync/issues/51), where a fresh sync warned that 588 files "appear deleted on the remote" while every one of them was still present and viewable in Nextcloud. The plugin only ever checked whether the server's response arrived with the right status code; it never checked whether the response itself could actually be read. A response that arrived cut short, or came back as an error page instead of the expected listing, parsed to the same thing as a server genuinely saying "this folder is empty" — and on a vault with hundreds of files, that triggered the safety warning falsely. Nothing was ever deleted (the built-in safety limit and per-file server checks caught it, exactly as designed), but the sync could not complete and the warning was confusing. The plugin now tells the two situations apart: a response it cannot make sense of is treated as a failed attempt to check the server, not as the server's answer, and the sync simply tries again next time.
