...
## What's new in this release (1.0.5)

- **Fixed: notes vanished from the server after the plugin moved a folder to the trash (1.0.5)** — a follow-up to [#46](https://github.com/siosig/obsidian-nextcloudsync/issues/46), where notes turned up in both the local trash and Nextcloud's own deleted files. Moving a folder to the trash takes its contents with it, but the plugin only forgot the folder itself and kept a record of every note inside. On the next sync that record read as "this note was synced and you have since deleted it here", so the deletion was passed on to the server for real. A trashed folder now forgets everything inside it at the same moment, which means that if the folder should never have been trashed, the next sync simply downloads the notes back.
- **Fixed: a note the server still had could be deleted without ever being asked about (1.0.5)** — when a note was gone from your device and also missing from the listing the server sent back, the plugin deleted it outright, assuming a note absent from the listing could not be on the server either. It now asks the server about that one note first: genuinely gone means there is nothing to delete, present and unchanged means the deletion is passed on as before, and edited on another device means the newer version is brought back to you instead of destroyed. On a healthy connection this costs no extra time.
- **Fixed: a listing the server could not answer properly was read as "there is nothing here" (1.0.5)** — reported in [#51](https://github.com/siosig/obsidian-nextcloudsync/issues/51), where a fresh sync warned that 588 files appeared deleted while every one of them was still on the server. The plugin checked the response's status code but never whether the response itself could be read, so a reply that arrived cut short — or came back as an error page — looked exactly like a server reporting an empty folder. Nothing was ever deleted, but the sync could not complete. Such a response is now treated as a failed attempt to reach the server, and the sync tries again next time.
