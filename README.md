...
## What's new in this release (1.0.4-beta.1)

- **Fixed: deleting the last note in a vault never reached your other devices (1.0.4-beta.1)** — reported in [#50](https://github.com/siosig/obsidian-nextcloudsync/issues/50). When a full sync found the server listing empty, the plugin skipped its "these files are gone from the server, remove them here too" step entirely. So if every file it was tracking had been deleted elsewhere — easiest to hit in a small vault — the notes stayed on disk, and because they also stayed in the plugin's index, the next sync rebuilt the listing from that index and concluded they were still on the server. It never recovered on its own. The two protections that actually matter are unchanged: a limit that refuses a suspiciously large batch of deletions, and a direct check with the server for every single candidate before anything is removed. An empty listing simply is not a failure any more: a listing that fails now raises an error instead of quietly looking empty.
- **Fixed: a vault folder deleted on the server is now put back, not obeyed (1.0.4-beta.1)** — the same investigation found that if the vault folder itself was missing from the server, the plugin did nothing at all: files were neither uploaded nor removed, and the vault sat in limbo. A missing folder says nothing about any individual file — a rename, a permissions change or a half-finished migration all look the same — and you never chose that folder's remote path; the plugin derives it from the vault name. So it is now repaired rather than obeyed: the plugin re-creates the folder and re-uploads this vault from your device, deleting nothing locally. It only does so once the server confirms the folder really was absent; if it turns out to be there after all, nothing is changed and the next sync tries again.
